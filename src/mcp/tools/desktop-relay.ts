...
export const DESKTOP_SESSION_MESSAGE_TOOL = {
  ...
  description: "仅Owner可用：SendMessage 找不到对方时用。替她给本机某个 Claude 窗口（Desktop session）带话，或把活派给常驻传话员会话（target=传话员，content=任务书），由那边按标题转交；回话 / 传话员汇报会以 trigger=desktop-reply 回到本频道。",
  inputSchema: {
    type: "object" as const,
    properties: {
