...
const PORT = 9337;
const LOCK = PROFILE + '.lock'; // 同一时刻只允许一份在跑（--wait-bg 最长 10 分钟）
...
async function main() {
  ...
  try {
    ...
    if (!waitBg) {
      ...
      // 已有飞书登录态 → 几秒内自动授权跳回站点
      for (let i = 0; i < 12; i++) { await sleep(1000); const tok = await tryFinish(cdp); if (tok) { cleanup(pid); return out({ ok: true, exp: tokenExp(tok), refreshed: true }); } }
      // 停在飞书登录页 → 截二维码，后台继续等
